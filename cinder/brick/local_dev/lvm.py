...
class LVM(executor.Executor):
    ...
    def activate_vg(self):
        """Activate the Volume Group associated to this instantiation (VG)
        ...
        try:
            ...
            LOG.exception(_('Error activating Volume Group'))
    ...
    def deactivate_vg(self):
        """Deactivate the Volume Group associated to this instantiation (VG)

        This should have the effect to force LVM to release any reference to
        the device.
        ...
        try:
            ...
            LOG.exception(_('Error deactivating Volume Group'))
            LOG.error(_('Cmd     :%s') % err.cmd)
            LOG.error(_('StdOut  :%s') % err.stdout)
            LOG.error(_('StdErr  :%s') % err.stderr)
            raise
    ...
    def destroy_vg(self):
        """Destroy the Volume Group associated to this instantiation (VG)
        ...
        try:
            ...
            LOG.exception(_('Error destroying Volume Group'))
            LOG.error(_('Cmd     :%s') % err.cmd)
            LOG.error(_('StdOut  :%s') % err.stdout)
            LOG.error(_('StdErr  :%s') % err.stderr)
            raise
    ...
    def pv_resize(self, pv_name, new_size_str):
        ...
        try:
            ...
            LOG.exception(_('Error resizing physical volume'))
            LOG.error(_('Cmd     :%s') % err.cmd)
            LOG.error(_('StdOut  :%s') % err.stdout)
            LOG.error(_('StdErr  :%s') % err.stderr)
            raise
